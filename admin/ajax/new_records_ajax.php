...
if(isset($_POST['add_class_name'])){
    $frm_data = filteration($_POST);

    $q = "INSERT INTO `class_name`(`name`) VALUES (?)";
    $values=[$frm_data['name']];
    $res=insert($q,$values,'s');
    echo $res;
...
if(isset($_POST['get_class_name'])){
    ...
    while($row = mysqli_fetch_assoc($res)){
        ...
        <td>$row[name]</td>
        <td>
         <button type="button" onclick="rem_class_name($row[id])" class="btn btn-danger btn-sm shadow-none">
         <i class="bi bi-trash"></i>Delete
        </button>
        </td>
        </tr>
...
if(isset($_POST['rem_class_name'])){
    $frm_data = filteration($_POST);
    $values = [$frm_data['rem_class_name']];

 
    $q = "DELETE FROM `class_name` WHERE `id`=?";
    $res = delete($q, $values,'i');
    echo $res;
}
